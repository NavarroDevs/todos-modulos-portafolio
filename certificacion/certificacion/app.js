...
const {getPublicaciones,getPublicaciones2, getUsuarioByEmailAndPassword, getCategorias, addPublicacion, getCategoriaByName, getPublicacionById, getComentarios, addComentario, getCantidadComentarios, addUsuario } = require('./consultas.js')
...
//aqui limito Imagen de la Noticia. Tamaño Máximo 2mb 
//como lo pide el enunciado
let limiteMb = 2;
app.use(fileUpload({
...
//RUTAS DE LAS VISTAS
// cargo la vista de manera descendente los años
...
// cargo la vista de manera ascendente los años
app.get("/mostrar", async (req, res) => {
    try {
        let publicaciones = await getPublicaciones2();
        publicaciones = publicaciones.map(publicacion => {
            publicacion.fecha = moment(publicacion.fecha).format('DD-MM-YYYY')
            return publicacion
        })
...
app.get("/publicar", verificarToken, async (req, res) => {
    ...
    try {
        ...
        console.log(error)
        res.render("publicar",{
            error: "Error no se han podido cargar los datos."
        })
    }
})
