...
const database = process.env.DB_DATABASE|| "blog_libros"

const pool = new Pool({
  host: 'localhost',
  user: 'postgres',
  password: '123456',
  database: 'blog_libros',
  max: 20,
  port: 5432,
...
//ordenar las publicaiones orden descendente
...
//ordenar las publicaiones orden ASCENDENTE
const getPublicaciones2 = async () => {
    let consulta = `
                    select p.id, p.titulo, p.contenido, p.fecha, p.imagen, u.nombre as autor from publicaciones p
                    join usuarios u
                    on p.id_usuario = u.id
