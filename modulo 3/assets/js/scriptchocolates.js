...
if (localStorage.getItem("productos3")) {
  productosCarro = JSON.parse(localStorage.getItem("productos4"));
  console.log(productosCarro);
  actualizarCarro(productosCarro);
}

cargarProductos(productos4);

function cargarProductos(listadoProductos) {
  let acumulador = "";
  listadoProductos.forEach((producto4) => {
    let template = `
    ...
    <div class="card text-white bg-secondary mb-3 card border-primary mb-3" style="width: 18rem">
        ...
        <div class="card-body">
          <h5 class="card-title">${productos4.nombre}</h5>
          <p class="card-text">${producto4.descripcion}</p>
          <p class="card-text">Precio Normal: $ ${producto4.precio}</p>
          <p class="card-text text-info">Descuento: - $ ${
            producto4.descuento
          }</p>
          <p class="card-text text-warning">Precio final: $ ${
            producto4.precio - producto4.descuento
          }</p>
          <a class="btn btn-primary" data-id="${
            producto4.id
          }" onclick="addToCart('${producto4.id}')">Comprar</a>
...
function actualizarCarro(listadoProductos) {
  ...
  const sumaProductos = listadoProductos.reduce(
    (acumulador, producto4) => acumulador + producto4.cantidad,
    valorInicial
  );
